...
async function Draw(){
  ...
  if(!t){
    ...
    stroke = (scol, fcol, lwo=1, od=true, oga=1, cp=true) => {
      if(scol){
        if(cp) x.closePath()
    ...
    init = (cl,rw,br) => {
      ...
      B = Array(rw*cl*br).fill().map((v,i)=>{
        X = ((i%cl)-cl/2+.5)*sp - ((i/cl|0)%2?0:sp/2)+sp/4
        Y = (((i/cl|0)%rw)-rw/2+.5)*sp/(2**.5*2)
        Z = ((i/cl/rw|0)-br/2+.5)*sp/(2) + sp*2
        return [X, Y, Z]
      })
      
      a = JSON.parse(JSON.stringify(B)).map(v=>{
        v[2] *=-1
        return v
      })
      B = [...B, ...a]
      shp = TruncatedOctahedron(1)
    }
    ...
    camx = camy = 0
    camz = -20
    pvx=pvy=pvz=prl=ppt=pyw=ywv=ptv=rlv=0
    Rl=0, Pt=0, Yw=Math.PI/4
    tracking = .02
    mtracking = .01
    sdist = .2
    ...
    view2.width = c.width
    view2.height = c.height
    vctx = view2.getContext('2d')
    
    vx=vy=vz=vx_=vy_=vz_=0
    
    iQueuec = 25
    queue = Array(iQueuec).fill().map(v=>{
      return Rn()*B.length | 0
    })
  }
  ...
  ty = B[curCell][1]
  tz = B[curCell][2]
  p1_ = -Math.atan2(tx-camx,tz-camz)
  if(p1_ > Math.PI) p1_ -= Math.PI*2
  if(p1_ < -Math.PI) p1_ += Math.PI*2
  p2_ = -Math.PI/2+Math.acos((ty-camy)/Math.hypot(tx-camx,ty-camy,tz-camz))
  if(Yw > Math.PI) Yw -= Math.PI*2
  if(Yw < -Math.PI) Yw += Math.PI*2
  ywv -= (Yw-p1_)*tracking
  ywv /= 1.25
  ptv -= (Pt-p2_)*tracking
  ...
  Yw += ywv
  Pt += ptv
  d = Math.hypot(a=tx-camx,b=ty-camy,e=tz-camz)
  vx_ /= 1.1
  vy_ /= 1.1
  vz_ /= 1.1
  if(d <= sdist+.01){
    q = Math.atan2(a,e)+.002
    if(q > Math.PI) q -= Math.PI*2
    if(q < -Math.PI) q += Math.PI*2
    d2 = Math.hypot(a,e)
    X = S(q)*d2
    Y = b
  ...
  if(!((t*60|0)%switchCellFreq)){
    ct = 0
    do{
      ct++
      newCell = B.length*Rn()|0
    }while(newCell==queue[queue.length-1] && ct<100);
    queue = [newCell, ...queue]
    curCell = queue.pop()
  }
  ...
  B.map((n,idx) => {
    ...
    Y = ty = n[1] - camy
    Z = tz = n[2] - camz
    d1 = Math.hypot(tx,ty,tz)
    R(Rl,Pt,Yw,1)
    if(d1<1 && Z>0){
      l = Q()
      s = Math.min(3e3,300/Z)
      x.drawImage(starImgs[1].img,l[0]-s/2,l[1]-s/2,s,s)
      x.fillStyle = '#fff4'
      x.font = s/2 + 'px courier'
      x.fillText(idx,l[0],l[1]+s/6)
    }
    shp.map((v,i)=>{
      x.beginPath()
      v.map((q,j)=>{
      ...
      stroke(d1<1?'#f002':'','#ff000008',.5,true,Z=1)
    })
  })
  
  x.beginPath()
  x.lineTo(960-140,540)
  x.lineTo(960+140,540)
  Z=5
  stroke('#0f86','',.7,true,1,0)
  x.beginPath()
  x.lineTo(960,540-140)
  x.lineTo(960,540+140)
  Z=5
  stroke('#0f86','',.7,true,1,0)
  
  vctx.drawImage(c,0,0,c.width,c.height)
  ...
  queue.map((v,i)=>{
    x.fillStyle = '#40f'
    x.fillText(`target cell: ${v}`,10,1080-40-queue.length*40+i*40)
  })
  
  x.fillStyle = '#fff'
  x.fillText(`target cell: ${curCell}`,10,1080-40)
  x.textAlign = 'center'
  x.beginPath()
  x.lineTo(420+20,1030+20)
  x.lineTo(420,1030)
  Z = 10
  tJoin = x.lineJoin
  x.lineJoin = x.lineCap = 'round'
  stroke('#4f8','',1,true,1,false)
  x.beginPath()
  x.lineTo(420+20,1030-20)
  x.lineTo(420,1030)
  x.lineTo(500,1030)
  stroke('#4f8','',1,true,1,false)
  x.lineJoin = x.lineCap = tJoin
  
  B.map((n,idx) => {
    X = tx = n[0]
    Y = ty = n[1]
    Z = tz = n[2]
    d1 = Math.hypot(camx-X,camy-Y,camz-Z)
    R(Rl1,Pt1,Yw1,1)
    if(Z>0){
      l = Q()
      x.globalAlpha = .5
      s = Math.min(3e3,2500/Z)
      x.drawImage(starImgs[1].img,l[0]-s/2,l[1]-s/2,s,s)
      x.globalAlpha = 1
      x.fillStyle = '#000'
      x.font = (s/10|0) + 'px courier'
      x.fillText(idx,l[0],l[1]+s/32)
    }
    x.globalAlpha = 1
    shp.map((v,i)=>{
      x.beginPath()
      ...
      flashcol = `hsla(${100},${d1<.75?100:0}%,${d1<.75?50:50+S(t*64)*50}%,${d1<.75?.1:.1})`
      col1 = idx == curCell ? flashcol : (d1<.75 ? '#ff000008' : '')
      col2 = idx == curCell ? flashcol : '#ff000008'
  ...
  Y1 = Y = camy
  Z1 = Z = camz
  p1_ = Math.atan2(vx_,vz_)
  R(Rl1,Pt1,Yw1,1)
  if(Z>0){
    l = Q()
    s = Math.min(3e3,1e3/Z)
    x.drawImage(starImgs[5].img,l[0]-s/2.07,l[1]-s/2.07,s,s)
    x.beginPath()
    x.lineTo(...l)
    X = 0
    Y = -.5
  ...
  x.drawImage(view2,c.width-c.width/4-20,20,c.width/4,c.height/4)
  x.lineWidth = 3
  x.strokeStyle = '#40f8'
  x.strokeRect(c.width-c.width/4-20,20,c.width/4,c.height/4)
